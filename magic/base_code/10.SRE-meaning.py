...
#     shannon_measurement_1q[i, :] = measurement_outcome_allstab_shannon(evolved_density)[0]
...
for i, indices in enumerate(index):
    ...
    shannon_measurement_2q[i, :], list_strings = measurement_outcome_allstab_shannon(evolved_density)
...
for i in range(36, 45):
    ax0.plot(theta, 2 - shannon_measurement_2q[:, i], marker=marker_list[i], linestyle='dashed', markerfacecolor='none',
             label=list_strings[i])
ax0.set_xticks([0, pi/4,  pi/2, 3 * pi /4, pi], ['$0$', '$\\pi/4$', '$\\pi/2$', '$3\\pi/4$',  '$\\pi$'])

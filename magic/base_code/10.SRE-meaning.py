...
circuit0 = QuantumCircuit(4)
# circuit0.ry(pi/2, 1)
# circuit0.h(0)
# circuit0.cx(0, 1)
psi0_2q = Statevector.from_label('0000').evolve(circuit0)

for i, indices in enumerate(index):
    loger_main.info(f'Calculating state {i}/ {len(index) - 1}')
    circuit_2q = QuantumCircuit(4)
...
loger_main.info('calculation done')
info = 3 - shannon_measurement_2q[0, :]
indices = np.where(2.99 < info)[0]
for index in indices:
    print(list_strings_2q[index])

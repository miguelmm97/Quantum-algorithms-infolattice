...
# Loading data saved with the corresponding functions in the script save.py
...
def load_information_per_scale_folded(d, file_path):
    """
    Load information_per_scale data for a given index d.
    Returns:
        data: numpy array of shape (rows, 2*N+1)
        attrs: dict of metadata attributes (N, J, h, k)
    """
    group = '/information_per_scale_folded'
    dset = f'info_per_scale_folded_{d}'
...
def load_gamma_global(d, file_path):
    """
    Load gamma_global scalar for index d.
    Returns:
        gamma_global: float
        attrs: dict of metadata
    """
    group = '/gamma_global'
    name = f'gamma_global_{d}'
    ...
    with tb.File(file_path, mode='r') as h5f:
        try:
            ...
            raise KeyError(f"Array {group}/{name} not found in {file_path}")
        gamma_global = arr.read()
        attrs = {key: getattr(arr.attrs, key) for key in ['N', 'J', 'h', 'k']}
    return gamma_global, attrs


def load_Hamiltonian_X_op_expect(d, file_path):
    """
    Load Hamiltonian X_op expectation (complex) for index d.
    Returns:
        value: complex
        attrs: dict of metadata
    """
    group = '/Hamiltonian_X_ops'
    name = f'Hamiltonian_X_op_{d}'
...
if __name__ == "__main__":
    # Example usage
    file_path = 'results_Potts/lowEnergyStates_N4_J1.000_h0.000_k3.h5'
    d = 0
    ...
    lam, l_attrs = load_gamma_global(d, file_path)
    print(f"gamma_global_{d}: {lam}, attrs={l_attrs}")
    hG, hG_attrs = load_Hamiltonian_X_op_expect(d, file_path)
    print(f"Hamiltonian X_op expect for d={d}: {hG}, attrs={hG_attrs}")
